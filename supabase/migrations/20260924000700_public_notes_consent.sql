-- Consenso esplicito per le note facoltative che possono contenere allergie.
-- La vecchia RPC rimane temporaneamente per i deploy già in esecuzione; solo
-- la chiave server può invocarla e la nuova API usa la versione con consenso.

alter table public.reservations add column notes_consent_at timestamptz;

create function public.create_public_reservation_with_notes_consent(
  p_date date,
  p_service text,
  p_arrival_time time,
...
begin
  ...
  insert into public.reservations (
    ...
    reminder_opt_in, table_id, customer_id, source, notes_consent_at
  ) values (
...
revoke all on function public.create_public_reservation_with_notes_consent(date, text, time, smallint, text, text, text, text, boolean, boolean) from public, anon, authenticated;
grant execute on function public.create_public_reservation_with_notes_consent(date, text, time, smallint, text, text, text, text, boolean, boolean) to service_role;
